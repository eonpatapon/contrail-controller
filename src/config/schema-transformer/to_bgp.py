...
class VirtualNetworkST(DictST):
    ...
    def update_static_ip_routes(self, new_ip_routes):
        ...
        # build new list of static routes
        entries = StaticRouteEntriesType()
        for prefix, next_hop_ip in new_ip_routes.items():
            static_route = StaticRouteType(prefix=prefix, next_hop=next_hop_ip)
            entries.add_route(static_route)

        # keep service instance routes
        for sre in primary_ri.obj.get_static_route_entries() or StaticRouteEntriesType():
            for sr in sre.get_route() or []:
                try:
                    netaddr.IPAddress(sr.next_hop)
                except ValueError:
                    entries.add_route(sr)

        primary_ri.obj.set_static_route_entries(entries)
